...
IntAct PPI（実験的証拠）
    ...
    配列相同性検索（どのPDBに対応するか）
        ↓
    SIFTS統合（同一PDB内の両チェーン）
        ↓
    物理接触フィルタ（実際に接触しているか）
        ↓
    tight/BSAフィルタ（生物学的に妥当な界面か）
        ↓
    MMR Top-K選抜（質×多様性のバランス）
        ↓
    QC（品質保証）
```

この一連の処理により、**「タンパク質AとBが相互作用する」という知識**から、**「その相互作用を構造的に代表するPDBテンプレートK件」**を厳密に選出できる。これは構造予測、ドッキング、創薬ターゲット探索などの下流解析に直接利用できる高品質なデータセットを構築する。
